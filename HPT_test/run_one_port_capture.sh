...
if [[ "$#" -ne 2 ]]; then
	echo "Usage: $0 <rate (in mbps)> <used packet size>"
	exit 1
fi
...
ipg=$((int(rint($(echo "scale=30; ($size * 8 / $rate) * 1000" | bc)))))
echo "c"
num_packets=$((int(rint($(echo "scale=30; $target_time * 1000000000 / ($ipg + ((8 * $size) / 10))" | bc)))))
...
./HPT_move_capture.sh "${size}B" $capfile --zip
