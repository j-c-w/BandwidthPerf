...
for size in $(seq $starting_size $increase $final_size); do
	...
	remote_run_script $HPTMachine hpt/record_port.sh $exa_port0 /root/jcw78/nvme/size_scan/${size}_psize.erf $cpus /root/jcw78/nvme/size_scan/${size}_cmd_out
	...
	if (( last_compress < size - compress_step )) || (( size == final_size )); then
		...
		remote_run_command $HPTMachine "rm /root/jcw78/nvme/size_scan/{$(seq -s, $last_compress $size)}_psize.erf-0.expcap"

		last_compress=$((size + 1))
	fi
done
